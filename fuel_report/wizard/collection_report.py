...
class CollectionReport(models.TransientModel):
    ...
    def collection_excel_report(self):
        ...
        sheet = workbook.add_worksheet('Collection Report - 27')
        ...
        check_date = datetime.now().replace(day=27)

        # Get the 27th of the previous month
        if check_date.month == 1:
            prev_due_date = check_date.replace(year=check_date.year - 1, month=12, day=27)
        else:
            prev_due_date = check_date.replace(month=check_date.month - 1, day=27)


        loan_data = self.env['loan.order'].search(
            [('state', '=', 'active'),'|',('last_paid_installment_date', '<', prev_due_date),('remaining_amount', '>', 0)])
        ...
        for row, order in enumerate(loan_data, start=1):
            row_format = reschedule_format if order.is_reschedule else content_format
            if order.is_reschedule:
                if isinstance(order.reschedule_installment_start_date, date):  # Check if installment_start_date is a date object
        ...
        for row, order in enumerate(loan_data, start=1):
            row_format = reschedule_format if order.is_reschedule else content_format
            if order.is_reschedule:
                if isinstance(order.reschedule_installment_end_date, date):  # Check if installment_end_date is a date object
        ...
        excel_report_id = self.env['save.ex.report.wizard'].create({
            "file_name": ex_report,
            "document_frame": "Collection Report - 27"
        })

...
import json  # Import json for encoding and decoding JSON data
import logging  # Import logging for generating log messages
...
def valid_response(data, status=200):
    ...
    return werkzeug.wrappers.Response(
        status=status,  # HTTP status code to indicate success
...
def invalid_response(typ, message=None, status=403):
    ...
    :param status: The HTTP status code for the response (default is 403 Forbidden).
    ...
    return werkzeug.wrappers.Response(
        status=status,  # HTTP status code to indicate error
        content_type="application/json; charset=utf-8",  # Set content type to JSON
        response=json.dumps(
            {
...
def invalid_error(data, status=500):
    """
    Create an HTTP response for server-side errors.

    Wraps error information in a JSON response object with a specified HTTP status code.

    :param data: The data to include in the error response.
    :param status: The HTTP status code for the response (default is 500 Internal Server Error).
    :return: A werkzeug Response object with JSON-encoded error information.
    """
    # If data is not a string, include a count of items; otherwise, set count to 1
    data = {"count": len(data) if not isinstance(data, str) else 1, "data": data}
    # Create and return a werkzeug Response object with JSON-encoded data
    return werkzeug.wrappers.Response(
        status=status,  # HTTP status code to indicate server error
        content_type="application/json; charset=utf-8",  # Set content type to JSON
        response=json.dumps(data, default=default),  # Encode data to JSON, using `default` function for serialization
    )
